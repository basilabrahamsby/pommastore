...
async def main():
    async with AsyncSessionLocal() as session:
        # Update order PS-2026-016 discount to 36.00 and total_amount to 79.50
        await session.execute(text("""
            UPDATE orders 
            SET discount_amount = 36.00, 
                total_amount = 79.50,
                updated_at = NOW()
            WHERE order_number LIKE '%PS-2026-016%' OR order_number LIKE '%016%'
        """))
        await session.commit()
        print("✓ Order PS-2026-016 updated successfully: discount_amount = 36.00, total_amount = 79.50")
